...
@section('content')
                    ...
                    <a href="{{route('sensoreCRUD.index')}}" class="btn btn-info pull-right">Back</a>
                    <h2>Modifica sensore</h2>
                    <h6 style="color: red">* campo obbligatorio</h6>          
                    <form action = "{{route('sensoreCRUD.update',[$sensor->id])}}" method = "post">
                        ...
                        <div class="form-group">
                            ...
                            <!--<input type="text" class="form-control" placeholder="Enter marca" name="marca" value= "{{$sensor->marca}}">-->
                            <!--@php if ($sensor->marca == "Acer") echo "selected='selected'";@endphp-->
                            <input type="text" class="form-control" list="marche" name="marca" value="{{$sensor->marca}}" autocomplete="off" placeholder="Seleziona una delle marche disponibili oppure inserisci una nuova marca...">
                        ...
                        <div class="form-group">
                            ...
                            <!--<input type="text" class="form-control" placeholder="Enter tipo" name="tipo" value="{{$sensor->tipo}}">-->
                            <!--@php if ($sensor->tipo == "Luce") echo "selected='selected'";@endphp-->
                            <input type="text" class="form-control" list="tipo" name="tipo" value="{{$sensor->tipo}}" autocomplete="off" placeholder="Seleziona uno dei tipi disponibili oppure inserisci un nuovo tipo...">
                            <datalist name="tipo" id="tipo">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" placeholder="Inserisci modello..." name="modello" value="{{$sensor->modello}}">
                        </div>
                        <div class="form-group">
                            <label for="codice">* Codice:</label>
                            <input type="text" class="form-control" placeholder="Inserisci codice..." name="codice" value="{{$sensor->codice}}">
                        </div>
                        <!--<div class="form-group">
                            <label for="unitaMisura">Unità di misura:</label>
                            <input type="text" class="form-control" placeholder="Enter unità di misura" name="unitaMisura" value="{{$sensor->unitaMisura}}">
                        </div>-->
                        <div class="form-group">
                            <label for="descrizione">Descrizione:</label>
                            <br>
                            <textarea rows="5" name="descrizione" placeholder="Inserisci descrizione...">{{$sensor->descrizione}}</textarea>
                        </div>
                        <div class="form-group">
                        <label for="selImpianto">* Seleziona impianto:</label>
                        <select class="form-control" name="impianto">
                            @foreach($installations as $installation)
                            @if($installation->id == $sensor->idImpianto)
                                <option value="{{$installation->id}}" selected>Codice:{{$installation->id}} SitoMonitoraggio:{{$installation->sitoMonitoraggio}}</option>
                            @else
                                <option value="{{$installation->id}}">Codice:{{$installation->id}} SitoMonitoraggio:{{$installation->sitoMonitoraggio}}</option>
                            @endif
                            @endforeach
                        </select>
                        </div>
